...
library(readr)
# path <- "https://github.com/stricje1/VIT_University/blob/master/Crime_Analysis_Mapping/data/atlanta_crime_10yr.zip"
...
leaflet(data, width = "100%") %>% addTiles() %>%
  ...
  addLayersControl(
    ...
    plot <- ggplot(df_arrest_time_district, aes(x = Hour, y = DayOfWeek, fill = norm)) +
      geom_tile() +
      theme(axis.text.x = element_text(angle = 90, vjust = 0.6, size = 4)) +
      labs(x = "Hour of Arrest (Local Time)", y = "Day of Week of Arrest", title = "Police Arrests in Atlanta from 2014 to 2018
           by Time of Arrest, Normalized by Station") +
...
plot <- ggplot(df_arrest_time_month, 
      ...
      labs(x = "Hour of Arrest (Local Time)", y = "Day of Week of Arrest",
           title = "Police Arrests in Atlanta from 2009 to 2018 
           by Time of Arrest, Normalized by Month") +
